...
class DataProcessor(Dataset):
    def __init__(self, root_path, file_list, category_list, pc_folder = "/txt/"):
        ...
        for file in self.list_of_names:
            # prefer fast .npy format (same content as the original _pc.txt)
            npy = self.root_path + self.pc_folder + file + "_pc.npy"
            if os.path.isfile(npy):
                samples = np.load(npy).astype(np.float32)
            else:
