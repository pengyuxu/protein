...
def RI_features(xyz, norm, new_xyz, new_norm, idx, group_all=False):
    ...
    dots_sorted, idx_ordered = order_index(xyz, new_xyz, new_norm[:,:,:3], idx)

    epsilon=1e-7
    #print(xyz.shape, idx_ordered.shape)
    ...
    #grouped_chem = grouped_xyz_norm[:,:,:,3:]
    #grouped_chem = grouped_xyz_norm[:,:,:,3:] - new_chem.view(B, S, 1, 2)
    grouped_xyz_norm = grouped_xyz_norm[:,:,:,:3]
...
def sample_and_group(npoint, radius, nsample, xyz, norm):
    ...
    #print("####", idx.shape)
    #ri_feat, idx_ordered = RI_features(xyz, norm, new_xyz, new_norm[:,:,:3], idx)
    ri_feat, idx_ordered = RI_features(xyz, norm[:,:,:3], new_xyz, new_norm[:,:,:3], idx)
...
class RIConv2SetAbstraction(nn.Module):
    def __init__(self, npoint, radius, nsample, in_channel, mlp, group_all, bias = True):
        ...
        for out_channel in mlp_0:
            self.prev_mlp_convs.append(nn.Conv2d(last_channel, out_channel, 1, bias = bias))
    ...
    def forward(self, xyz, norm, points):
        ...
        if points is not None:  # transform from [B, C, N] to [B, N, C]
            ...
            #print(xyz.shape, norm.shape, points.shape)
        #else:
        ...
        #print(new_xyz.shape, new_norm.shape, ri_feat.shape)
        ...
        if points is not None:
            if idx_ordered is not None:
                #print("###", points.shape, idx_ordered.shape)
